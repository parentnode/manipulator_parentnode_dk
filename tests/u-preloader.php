	...
	Util.Objects["test"] = new function() {
		this.init = function(scene) {
			...
			scene.callback = function(queue) {
				if(u.qsa("li.loaded", queue[0]._preloader_queue).length == u.qsa("li", queue[0]._preloader_queue).length) {
					u.ac(queue[0]._preloader_queue, "correct");
				}
				else {
					u.ac(queue[0]._preloader_queue, "error");
				}
			}
